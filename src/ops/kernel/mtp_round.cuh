...
// Match: request-major fixed K=1..7 autoregressive MTP round transition.
...
__global__ void mtp_prepare_next_round_kernel(
    ...
    std::int32_t* ar_rope_positions, std::int32_t* ar_valid_columns, std::int32_t verify_k,
    std::int32_t proposal_k, std::int32_t ar_step_stride, std::int32_t max_context) {
    const int row = static_cast<int>(blockIdx.y);
    const int T   = verify_k + 1;
    int a         = accepted[row];
    a             = a < 0 ? 0 : (a > verify_k ? verify_k : a);
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        next                     = next < 0 ? 0 : (next > proposal_k ? proposal_k : next);
        next_extents[row]        = next;
        const int steps          = proposal_k > 1 ? proposal_k - 1 : 1;
